    ...
    def __init__(self, num_pop=1, pop_size=1):
        ...
        for pop_a in self.populations:
            for pop_b in self.populations:
                p = pynn.Projection(pop_a, pop_b, connector, receptor_type='excitatory')
                self.outgoing_connections['exc'].append(p)
                p = pynn.Projection(pop_a, pop_b, connector, receptor_type='inhibitory')
                self.outgoing_connections['inh'].append(p)
        ...
        for pop_a, pop_b in zip(self.populations, self.populations[1:]):
            ...
            self.incoming_connections['inh'].append(p)
        '''
...
class Input_neurons(Unconnected_neurons):
    ...
    def __init__(self, num_pop=1, pop_size=1, exc_spike_times=100):
        ...
        for p_stim, p_in in zip(self.input, self.populations):
            ...
            self.incoming_connections['exc'].append(p)
            p = pynn.Projection(p_stim, p_in, connector, receptor_type='inhibitory')
            self.incoming_connections['inh'].append(p)
