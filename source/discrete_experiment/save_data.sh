datevar=040509022020
mkdir $datevar
cp avg_fitness.svg config-ff ffnn.log Digraph.gv Digraph.gv.svg neat-checkpoint-3891 speciation.svg $datevar
git add -f save_data.sh avg_fitness.svg config-ff ffnn.log Digraph.gv Digraph.gv.svg neat-checkpoint-3891 speciation.svg evoalg.py game.py
...
git commit -m "3894 gen, low mutate rates, 150 pop, albantakis replication with only catch task"
